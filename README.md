...
### 1.1 프로젝트 배경
매일 수많은 경제 뉴스가 쏟아지지만, 이를 매일 읽는 것도 어렵고, 읽더라도 지식을 내 것으로 만들기 어렵습니다. <br>
본 프로젝트는 사용자가 뉴스를 수동으로 찾고 정리하는 번거로움을 해결하고, AI를 통해 학습 콘텐츠(퀴즈)로 변환하여 배달하는 것을 목표로 합니다.

### 1.2 사용 기술 (표 사용해서 만들면 좋을듯)
1. n8n
2. DB(PostgreSQL)
3. LLM
4. Discord
...
### 1.3 핵심 프로세스
1. 데이터 수집: n8n을 이용하여 신뢰도 높은 경제 뉴스 사이트에서 최신 기사를 크롤링합니다.
2. AI 콘텐츠 생성: Groq(LLM)을 활용해 기사 본문을 분석하고, 정형화된 JSON 형태의 퀴즈를 생성합니다.
3. 데이터 적재: Supabase(PostgreSQL)에 기사와 퀴즈를 저장합니다.
4. 조건부 알림 송신: DB에 학습 데이터가 충분히 쌓였을 때 디스코드로 알림을 보내 사용자 피로도를 낮춥니다.

## 2. 주요 기능 및 구현 상세
                    기능 별로 사진 첨부
1. **뉴스 크롤링 및 데이터 가공:**
   - 정해진 스케줄에 따라 기사 목록을 수집하고, Split Out 노드를 통해 대량의 데이터를 개별 처리 단위로 분리합니다.
   - HTML Parser를 활용하여 불필요한 태그를 제거하고 LLM이 처리하기 적합한 순수 텍스트 본문만 추출합니다.
2. **데이터 적재:**
3. ....
...
3. **알림 중복 발송 문제**
     - **문제 상황:** 퀴즈의 개수를 확인하기 위해 count(*) 를 수행하면 모든 문제의 수가 아닌 기사에 있는 문제 수가 각각 계산되어 알림이 기사의 수만큼 가는 문제가 발생했었다.
     - **원인 분석:** 
     - **해결 방법:** 
...
경제 뉴스 데이터를 수집하고 AI가 생성한 퀴즈를 데이터베이스에 저장하는 **자동화 파이프라인 구현**을 담당했습니다.
